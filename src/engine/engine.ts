 ...
 */
import { ActionType, HandPlayer, HandState, LegalAction, LogEntry, Pot, TableState } from "./types.js";
import { freshDeck, Rng, CryptoRng } from "./cards.js";
import { compareHands, describeHand, evaluateBest } from "./evaluator.js";

export class EngineError extends Error {
  constructor(
    message: string,
    readonly code: string = "illegal-action",
  ) {
    super(message);
...
export interface EngineEvent {
  ...
  action?: { playerId: string; type: ActionType; amount: number; allIn: boolean };
  ...
  results?: { playerId: string; seat: number; stackBefore: number; stackAfter: number }[];
}
...
export class PokerEngine {
  ...
  startHand(state: TableState): EngineOutcome {
    ...
    if (state.hand !== null) throw new EngineError("hand already in progress", "hand-in-progress");
    const participants: { seat: NonNullable<TableState["seats"][number]>; seatIndex: number }[] = [];
    for (let seatIndex = 0; seatIndex < state.seats.length; seatIndex++) {
      const seat = state.seats[seatIndex];
      if (seat !== null && seat !== undefined && seat.stack > 0) participants.push({ seat, seatIndex });
    }
    ...
    for (let i = 0; i < 2; i++) {
      for (const hp of hand.players) hp.holeCards.push(hand.deck.shift()!);
    }
    ...
    events.push(
      this.logEvent(
        state,
        `Hand #${state.handNumber} started — dealer: ${sbPlayer.nickname}. Blinds ${state.smallBlind}/${state.bigBlind}.`,
        hand.handId,
      ),
    );
  ...
  applyAction(state: TableState, seat: number, type: ActionType, amount?: number): EngineOutcome {
    const events: EngineEvent[] = [];
    const hand = state.hand;
    if (hand === null) throw new EngineError("no hand in progress", "no-hand");
    const p = this.playerAtSeat(hand, seat);
    if (p === undefined) throw new EngineError("player not in hand", "not-in-hand");
    if (p.folded || p.excluded) throw new EngineError("player is not active", "not-active");
    if (p.allIn) throw new EngineError("player is all-in", "all-in");
    if (hand.currentTurnSeat !== seat) throw new EngineError("not your turn", "not-your-turn");
    ...
    switch (type) {
      ...
      case "call": {
        ...
        events.push(
          this.logEvent(
            state,
            p.stack === 0 ? `${p.nickname} calls all-in (${paid}).` : `${p.nickname} calls ${paid}.`,
            hand.handId,
          ),
        );
        break;
      ...
      case "bet": {
        if (hand.toCall !== 0) throw new EngineError("cannot bet when facing a bet", "illegal-bet");
      ...
      case "raise": {
        if (hand.toCall === 0) throw new EngineError("cannot raise without a bet to raise", "illegal-raise");
        ...
        if (p.acted) throw new EngineError("betting is not open for this player (short all-in did not reopen)", "illegal-raise");
        paid = total - p.bet;
        this.wager(p, paid);
        newLevel = total;
    ...
    if (newLevel !== null && newLevel > prevToCall) {
      ...
      if (increment >= prevMinRaise) {
        ...
        for (const other of hand.players) {
          if (other.playerId !== p.playerId && !other.folded && !other.allIn && !other.excluded) other.acted = false;
        }
      }
  ...
  removePlayer(state: TableState, playerId: string, reason: string): EngineOutcome {
    const hand = state.hand;
    if (hand === null) return { events: [], changed: false };
    const p = hand.players.find((x) => x.playerId === playerId);
    if (p === undefined || p.folded || p.excluded) return { events: [], changed: false };
    const before = this.totalChips(state);
    p.folded = true;
    p.acted = true;
    const events: EngineEvent[] = [this.logEvent(state, `${p.nickname} left the hand (${reason}).`, hand.handId)];
    this.advanceIfRoundClosed(state, events);
    this.assertConservation(state, before);
    return { events, changed: true };
  }
  ...
  autoAct(state: TableState, at?: number): EngineOutcome {
    ...
    const hand = state.hand;
    if (hand === null) return { events: [], changed: false };
    if (hand.turnDeadlineAt === 0 || now < hand.turnDeadlineAt) return { events: [], changed: false };
    const p = this.playerAtSeat(hand, hand.currentTurnSeat);
    if (p === undefined || p.folded || p.allIn || p.excluded) return { events: [], changed: false };
    const before = this.totalChips(state);
    const type: ActionType = hand.toCall > 0 ? "fold" : "check";
    const events: EngineEvent[] = [];
    if (type === "fold") {
      p.folded = true;
  ...
  totalChips(state: TableState): number {
    ...
    for (let i = 0; i < state.seats.length; i++) {
      ...
      if (hp !== undefined) total += hp.stack + hp.committed + hp.bet;
      else total += seat.stack;
    }
    return total;
  }
  ...
  private logEvent(state: TableState, text: string, handId?: string): EngineEvent {
    const at = this.now();
    const entry: LogEntry = { at, text, ...(handId !== undefined ? { handId } : {}) };
  ...
  private wager(p: HandPlayer, amount: number): void {
    if (amount < 0 || amount > p.stack) throw new EngineError(`invalid wager ${amount}`, "illegal-amount");
    p.stack -= amount;
    p.bet += amount;
    if (p.stack === 0) p.allIn = true;
  }

  private postBlind(hand: HandState, p: HandPlayer, amount: number): void {
    if (amount < 0 || amount > p.stack) throw new EngineError("invalid blind", "illegal-blind");
    p.stack -= amount;
    p.bet += amount;
    if (p.stack === 0) p.allIn = true;
  ...
  private requireInteger(value: number | undefined, what: string): number {
  ...
  private indexOf(hand: HandState, playerId: string): number {
    return hand.players.findIndex((p) => p.playerId === playerId);
  }

  private playerAtSeat(hand: HandState, seat: number): HandPlayer | undefined {
    return hand.players.find((p) => p.seat === seat);
  }

  private nextOccupiedSeat(state: TableState, from: number): number {
    for (let i = 1; i <= state.maxSeats; i++) {
      const seat = (from + i + state.maxSeats) % state.maxSeats;
      const s = state.seats[seat];
      if (s !== null && s !== undefined && s.stack > 0) return seat;
    }
    return -1;
  }

  /** First non-folded, non-all-in, non-excluded player after `from` (clockwise). */
  private firstActiveAfter(hand: HandState, from: number): number {
    ...
    for (let i = 1; i <= players.length; i++) {
      ...
      if (p !== undefined && !p.folded && !p.allIn && !p.excluded) return p.seat;
    }
    return -1;
  }

  private armTurn(state: TableState, hand: HandState): void {
    hand.turnDeadlineAt = hand.currentTurnSeat >= 0 ? this.now() + state.actionTimeoutMs : 0;
  }

  private advanceIfRoundClosed(state: TableState, events: EngineEvent[]): void {
    const hand = state.hand;
    if (hand === null) return;
    ...
    const roundClosed =
      active.every((p) => p.acted) && active.every((p) => p.bet === hand.toCall || hand.toCall === 0);
    if (!roundClosed) {
  ...
  private completeStreet(state: TableState, events: EngineEvent[]): void {
    const hand = state.hand;
    if (hand === null) return;
  ...
  private resetRound(state: TableState): void {
    const hand = state.hand;
    if (hand === null) return;
    hand.toCall = 0;
    hand.minRaise = state.bigBlind;
  ...
  private runOut(state: TableState, events: EngineEvent[]): void {
    const hand = state.hand;
    if (hand === null) return;
    for (const p of hand.players) {
      p.committed += p.bet;
  ...
  private endHandBySurvivor(state: TableState, events: EngineEvent[]): void {
    const hand = state.hand;
    if (hand === null) return;
    const survivor = hand.players.find((p) => !p.folded && !p.excluded);
    const { pots, refunds } = buildPotsFrom(
      hand.players.map((p) => ({ playerId: p.playerId, amount: p.committed + p.bet, folded: p.folded || p.excluded })),
    );
  ...
  private runShowdown(state: TableState, events: EngineEvent[]): void {
    const hand = state.hand;
    if (hand === null) return;
    hand.phase = "showdown";
    for (const p of hand.players) {
    ...
    const { pots, refunds } = buildPotsFrom(
      hand.players.map((p) => ({ playerId: p.playerId, amount: p.committed, folded: p.folded || p.excluded })),
    );
    hand.pots = pots;
    const revealed: RevealEntry[] = [];
    const winners: WinnerEntry[] = [];
    ...
    for (const pot of pots) {
      const eligible = pot.eligiblePlayerIds
        ...
        .filter((p): p is HandPlayer => p !== undefined);
  ...
  private finishHand(state: TableState, winners: WinnerEntry[], events: EngineEvent[]): void {
    ...
    const winById = new Map<string, number>();
    for (const w of winners) winById.set(w.playerId, (winById.get(w.playerId) ?? 0) + w.amount);
    const handId = hand.handId;
    state.lastShowdown = {
      handNumber: hand.handNumber,
      handId,
      reveal: (events.find((e) => e.kind === "showdown")?.reveal ?? []) as RevealEntry[],
  ...
  private seatOrderFrom(dealerSeat: number, a: number, b: number): number {
    const distA = (a - dealerSeat - 1 + 100) % 100;
    const distB = (b - dealerSeat - 1 + 100) % 100;
    return distA - distB;
  }

  private assertConservation(state: TableState, before: number): void {
 ...
 */
export function buildPotsFrom(
  contributors: { playerId: string; amount: number; folded: boolean }[],
): { pots: Pot[]; refunds: { playerId: string; amount: number }[] } {
  ...
  for (const level of levels) {
    ...
    if (eligible.length > 0) {
      ...
      for (const s of shares) {
        if (s.share > 0) refunds.push({ playerId: s.playerId, amount: s.share });
      }
    }
    prev = level;
  }
