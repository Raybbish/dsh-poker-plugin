...
import { DECK_SIZE, MAX_RANK, MIN_RANK, SUIT_COUNT } from "./types.js";
/** Crypto-backed uniform int via rejection sampling on randomBytes. */
export class CryptoRng {
    nextInt(n) {
        if (n <= 0)
            throw new RangeError("nextInt(n) requires n > 0");
...
export function buildDeck() {
    ...
    if (deck.length !== DECK_SIZE)
        throw new Error("deck build invariant violated");
    return deck;
}
/**
 ...
 */
export function shuffle(deck, rng = new CryptoRng()) {
...
export function deal(deck, count) {
    if (count > deck.length)
        throw new Error("deal: not enough cards");
    return deck.splice(0, count);
}
/** Fresh, shuffled full deck. */
export function freshDeck(rng = new CryptoRng()) {
