...
(function (HandCategory) {
    HandCategory[HandCategory["HIGH_CARD"] = 0] = "HIGH_CARD";
    HandCategory[HandCategory["PAIR"] = 1] = "PAIR";
    HandCategory[HandCategory["TWO_PAIR"] = 2] = "TWO_PAIR";
    HandCategory[HandCategory["THREE_OF_A_KIND"] = 3] = "THREE_OF_A_KIND";
    HandCategory[HandCategory["STRAIGHT"] = 4] = "STRAIGHT";
    HandCategory[HandCategory["FLUSH"] = 5] = "FLUSH";
    HandCategory[HandCategory["FULL_HOUSE"] = 6] = "FULL_HOUSE";
    HandCategory[HandCategory["FOUR_OF_A_KIND"] = 7] = "FOUR_OF_A_KIND";
    HandCategory[HandCategory["STRAIGHT_FLUSH"] = 8] = "STRAIGHT_FLUSH";
...
export function evaluateFive(cards) {
    ...
    const isStraight = (rs) => {
        ...
        if (uniq.length === 5 && uniq[0] - uniq[4] === 4)
            return { ok: true, high: uniq[0] };
        const wheel = [14, 5, 4, 3, 2];
        if (uniq.length === 5 && wheel.every((r) => uniq.includes(r)))
            return { ok: true, high: 5 };
        return { ok: false, high: 0 };
    };
    const straight = isStraight(ranks);
    if (flush && straight.ok)
        return [HandCategory.STRAIGHT_FLUSH, straight.high];
    const counts = new Map();
    for (const r of ranks)
        counts.set(r, (counts.get(r) ?? 0) + 1);
    const counted = [...counts.entries()]
        .map(([rank, count]) => ({ rank, count }))
        .sort((a, b) => b.count - a.count || b.rank - a.rank);
    const of = (n) => counted.find((c) => c.count === n);
    const four = of(4);
    const three = of(3);
    const pairs = counted.filter((c) => c.count === 2).map((c) => c.rank);
    const kickers = counted.filter((c) => c.count === 1).map((c) => c.rank);
    if (four)
        return [HandCategory.FOUR_OF_A_KIND, four.rank, kickers[0] ?? 0];
    if (three && pairs.length > 0)
        return [HandCategory.FULL_HOUSE, three.rank, pairs[0]];
    if (flush)
        return [HandCategory.FLUSH, ...ranks];
    if (straight.ok)
        return [HandCategory.STRAIGHT, straight.high];
    if (three)
        return [HandCategory.THREE_OF_A_KIND, three.rank, kickers[0] ?? 0, kickers[1] ?? 0];
    if (pairs.length >= 2)
        return [HandCategory.TWO_PAIR, pairs[0], pairs[1], kickers[0] ?? 0];
    if (pairs.length === 1)
        return [HandCategory.PAIR, pairs[0], kickers[0] ?? 0, kickers[1] ?? 0, kickers[2] ?? 0];
...
export function evaluateBest(cards) {
    ...
    for (const idx of COMBINATIONS_7) {
        ...
        if (best === null || compareHands(value, best) > 0)
            best = value;
    }
    return best;
}
...
export function describeHand(value) {
    const cat = value[0];
    const r = (i) => RANK_LABELS[value[i] ?? 0] ?? "?";
    switch (cat) {
